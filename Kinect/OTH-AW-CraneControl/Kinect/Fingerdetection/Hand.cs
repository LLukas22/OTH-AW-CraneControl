...
namespace Kinect.Fingerdetection
{
    ...
    public class Hand
    {
        internal Hand(ulong trackingID, HandState state, IList<DepthPointEx> contour, IList<DepthPointEx> fingers,
            CoordinateMapper coordinateMapper)
        {
            TrackingId = trackingID;

            //if (state == HandState.Lasso)
            //{
            ...
            var depths = contour.Select(d => (ushort)d.Z).ToArray();
            ...
            coordinateMapper.MapDepthPointsToCameraSpace((DepthSpacePoint[])ContourDepth, depths,
                (CameraSpacePoint[])ContourCamera);

            ContourColor = new ColorSpacePoint[ContourDepth.Count];
            coordinateMapper.MapDepthPointsToColorSpace((DepthSpacePoint[])ContourDepth, depths,
                (ColorSpacePoint[])ContourColor);
        }
