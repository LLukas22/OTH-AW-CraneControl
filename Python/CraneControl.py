...
from Modules.TCPServer import Server, Payload
...
class ThreadController(object):
    ...
    def __init__(self, labels, resolution, port):
        ...
        self.tcpServer = Server(
            int(port), labels, threshhold=min_conf_threshold)
...
class Window(object):
    ...
    def restartServer(self):
        ...
        self.threadController = ThreadController(
            labels, self.resolution, (int)(self.PortTxt.get("1.0", tk.END)))
        ...
        if(self.threadController is not None):
            if(self.threadController.hasCamera()):
                cv2image = cv2.cvtColor(
                    self.threadController.videostream.drawDetection(), cv2.COLOR_BGR2RGBA)
                ...
                img = Image.fromarray(
                    np.zeros((self.resolution[1], self.resolution[0], 3), np.uint8))
                imgtk = ImageTk.PhotoImage(image=img)
                app.picturebox.imgtk = imgtk
                app.picturebox.configure(image=imgtk)
                self.restartWebcam()
    ...
    def drawIndicatores(self):
        if(self.threadController is not None and self.threadController.tcpServer is not None):
            ...
            if(self.threadController.tcpServer.payload.power):
                self.PowerIndicator.config(image=self.icons.poweron)
            else:
                self.PowerIndicator.config(image=self.icons.poweroff)
