...
class Model(object):
    ...
    def scores(self, newscores):
            ...
            if validscores:
                ...
                if(current is not None):
                    if (current > self.threshold):
                        object_name = self.labels[int(self.classes[validscores.index(current)])]
                        switcher = {
                            'Up': HandStates.Up,
        ...
        ##Set bool Properties 
        self.resetDirection()
        if(self.handState != HandStates.Power): self.powerlock = False
        if(self.handState != HandStates.Toggle): self.reglerlock = False
        if(self.handState == HandStates.Power and not self.powerlock): 
            ...
            if(self.rightVelocity>100): self.rightVelocity = 100
            self.right = True
        elif(self.handState == HandStates.Left):
            self.rightVelocity = 0
            self.leftVelocity += self.acceleration
            if(self.leftVelocity>100): self.leftVelocity = 100
            self.left = True
        else:
            self.rightVelocity = 0
            self.leftVelocity = 0
    ...
    def package(self):
        ...
        if(not self.serverActive):return data
        ...
        if(self.regler):
            ...
            data[4]=2
        if(self.up): data[3]=1
        if(self.down): data[3]=2
        if(self.left): data[1]= self.leftVelocity
        if(self.right): data[2]= self.rightVelocity
        return data
    ...
    def frameWithDetections(self):
        if(self.hasFrame):  
            ...
            if(self.boxes is not None and self.classes is not None and self._scores is not None):
                if self.boxes.any() and self.classes.any() and self._scores.any():
                    for i,score in enumerate(self._scores):
                        if (score >  self.threshold and score <= 1.0):
                            ...
                            xmin = int(max(1, (self.boxes[i][1] * self.imageWidth)))
                            ymax = int(min(self.imageHeight, (self.boxes[i][2] * self.imageHeight)))
                            xmax = int(min(self.imageWidth, (self.boxes[i][3] * self.imageWidth)))

                            cv2.rectangle(detectionframe, (xmin, ymin),(xmax, ymax), (10, 255, 0), 2)
                            ...
                            cv2.rectangle(detectionframe, (xmin, label_ymin-labelSize[1]-10), (xmin+labelSize[0], label_ymin+baseLine-10), (255, 255, 255), cv2.FILLED)
                            # Draw label text
                            cv2.putText(detectionframe, label, (xmin, label_ymin-7),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)      
        else:
        ...
        if(cpuUsage > 90 or self.detectionTime > 200):
            color = (0, 0, 255)
        cv2.putText(detectionframe, 'Cpu:{0}%,{1:0.1f}ms'.format(cpuUsage,self.detectionTime), (30, 50),cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2, cv2.LINE_AA)
    ...
    def emptyImage(self):
        ...
        cv2.putText(image, text, (textX, textY ), font, fontsacle, color, thickness)
        return image
